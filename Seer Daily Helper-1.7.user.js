...
(function () {
    ...
    const autoFightStage = async (stageConfig, planetId) => {
        ...
        while (isAutoFighting) {
            ...
            const maxRounds = 5; // 防止死循环

            while (round < maxRounds && !battleEnded && isAutoFighting) {
                round++;
                logMessage(`🎯 ${name} 第 ${round} 轮释放技能...`, 'info');

                // 释放技能（使用 currentGroupId 作为 groupId）
                ...
                await delay(300); // 等待服务器响应

                // 检查是否收到战斗结束信号（从 enterStart 开始扫描）
                for (let i = capturedPackets.length - 1; i >= enterStart; i--) {
                    ...
                    if (pkt.cmd === 1056) {
                        try {
                            ...
                            if (result === 1) {
                                logMessage(`✅ ${name} 胜利（${round} 轮）`, 'success');
                                battleEnded = true;
                                break;
                            } else if (result === 2) {
                            ...
                            // result === 0 表示战斗继续，无需处理
                        } catch (e) {}
                    }
                    // 补充：有些关卡结束会直接发 cmd=1109（掉落）
                    if (pkt.cmd === 1109) {
                        logMessage(`✅ ${name} 战斗结束（收到掉落奖励）`, 'success');
                        battleEnded = true;
                        break;
                    }
                }
